...
function send_sms($formdata) {
    ...
    global $database, $mosConfig_absolute_path, $mosConfig_live_site, $_COMPANY_NAME, $_EMAIL, $_FOOTER1;
    global $mosConfig_smsuser, $mosConfig_smspaswd, $SMS_TEST_MODE,$mosConfig_msisdn;
    $database->setQuery("set names utf8");
    $database->query();
    $err = $result = "";
    foreach ($formdata as $key => $fdata) {
    ...
    foreach ($adresses as $to) {
        ...
        $data = 'username=' . $mosConfig_smsuser . '&password=' . $mosConfig_smspaswd . '&message=' . urlencode($message) . '&msisdn=' . $to . '&sender=' .$mosConfig_smsuser ;
