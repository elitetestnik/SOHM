...
function getUsername($email){
      ...
      $query = mysql_query("SELECT * FROM nor_promoters");
...
$message = '<html><head><title>'.$formdata['subject'].'</title><meta http-equiv="Content-Type" content="text/html; charset=utf8" /></head><body>'.$messageBody.'<br/>'.$_FOOTER1.'</body></html>';
        ...
        VALUES ( '1', '".$formdata['whos_id']."', '".$formdata['to_name']."', '".$formdata['to_type']."',  '".$to."', '".addslashes($formdata['subject'])."' ,  '".$messageBody."' ,  '0','".$attlinks."')";
        $database->setQuery( $query );
$database->query();
$id= $database->insertid();
$msg->From=$_EMAIL;
