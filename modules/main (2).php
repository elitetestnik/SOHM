...
function removeFromEnd($string, $stringToRemove) {
 ...
 function getUsername($email){
      ...
      $query = mysql_query("SELECT * FROM demo3_promoters");
...
$message = '<html><head><title>'.$formdata['subject'].'</title><meta http-equiv="Content-Type" content="text/html; charset=utf8" /></head><body>'."Dear ".addslashes($username)."!<br/>".$formdata['message'].'<br/>'.$_FOOTER1.'</body></html>';


$query="INSERT INTO `#__messages` (
`from` , `to_id` , `to_name` , `to_type` , `to_addr` , `subject` , `mesage_body` , `status`,`attaches` )
VALUES ( '1', '".$formdata['whos_id']."', '".$formdata['to_name']."', '".$formdata['to_type']."',  '".$to."', '".addslashes($formdata['subject'])."' ,  '".addslashes("Dear ".$username."!<br/>".$formdata['message'])."' ,  '0','".$attlinks."'
)";
$database->setQuery( $query );
$database->query();
$id= $database->insertid();
