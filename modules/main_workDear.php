...
if ($search!="") $add=" and ( i.company like '%".$search."%' or  i.contact_person like '%".$search."%')";
 $result.= "
  <table width='95%'  border='0' align='center' cellpadding='0' cellspacing='0'>
      <tr>
        <td width='53%' class='style4'>Inquires <span id='map_button'><button onclick='show_the_map();'>toggle inguries</button></span></td>
        ...
        <tr>
          ...
          <tr>
              ...
              <tr>
                ...
                <td height='36' colspan='4' bgcolor='#FFFFFF' class='style17'><div align='left' id='our_comment' name='our_comment'><span class='style34'>".$inquiry->comments2."</span></div>
                </td>
              </tr>
...
$result="<TEXTAREA NAME='message' id='message' ROWS='30' COLS='80' class='ckeditor'  >".$template."</TEXTAREA>";
$objResponse = new xajaxResponse('UTF-8');
...
$connection = mysqli_connect($mosConfig_host,$mosConfig_user,$mosConfig_password,$mosConfig_db) or die ('NO CONNECTION');
  $query = mysql_query("SELECT contact_person FROM demo3_promoters WHERE email='".$email."'");
 $fetch = mysql_fetch_array($query);
 $username=$fetch{'contact_person'};
...
$attlinks=$formdata['filenames'];$attlinks=str_replace($mosConfig_absolute_path,$mosConfig_live_site,$attlinks);
$to  = $formdata['to'];
$adresses=explode(',',$to);
...
$message = '<html><head><title>'.$formdata['subject'].'</title><meta http-equiv="Content-Type" content="text/html; charset=utf8" /></head><body>'."Dear ".getUsername(trim($to))."!<br/>".$formdata['message'].'<br>'.$_FOOTER1.'</body></html>';
...
`from` , `to_id` , `to_name` , `to_type` , `to_addr` , `subject` , `mesage_body` , `status`,`attaches` )
VALUES ( '1', '".$formdata['whos_id']."', '".$formdata['to_name']."', '".$formdata['to_type']."',  '".$to."', '".addslashes($formdata['subject'])."' ,  '".addslashes("Dear ".getUsername(trim($to))."!<br/>".$formdata['message'])."' ,  '0','".$attlinks."'
)";
$database->setQuery( $query );
$database->query();
$id= $database->insertid();
...
$settings = $database->loadObjectList();
$ft = new FastTemplate($mosConfig_absolute_path."/templates");
$subm="<INPUT TYPE='submit' value='Save settigns'>";
$sel="";
