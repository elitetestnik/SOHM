...
$hhh=array("ID","Promoter","Contact person","Weekend number","Address 1","Address 2","City code (ZIP)","Town",

    "Location","Country","Local phone","Cell phone","e-mail", "Website","Capacity","Type_of_music","Comments",

    "status","lastupdate","whosupdate","priority","coords","coords_crc");
...
$query="select p.id, p.name, p.contact_person, p.weeknum , p.street_addr1, p.street_addr2, p.city_code, p.town, p.location, ( select c.name from demo3_countries c where c.id= p.country) as country, p.phone1, p.phone2, p.email, p.website, p.capacity, p.typemusic , p.comments , p.status, p.lastupdate, p.whosupdate,  p.priority, p.coords, p.coords_crc FROM nor_promoters p WHERE 1=1";
