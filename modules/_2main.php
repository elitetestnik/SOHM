...
function getagentInfo($agent_id) {
    ...
    foreach ($agents as $agent) {
        ...
        if ("" == $agency_name)
            $agency_name = "<font color='red'>Undefined ...</font>";
...
function draw_menu($id) {
    ...
    foreach ($links as $link) {
        if ($id == $i)
            $result.="<tr><td height='30' class='active-nav'>" . $link . "</td></tr>";
        else
            $result.="<tr><td height='30' class='default-nav'>" . $link . "</td></tr>";
        $i++;
    }
...
function editagentInfo($agent_id = 0) {
    ...
    if ($agent_id > 0)
        $database->setQuery("SELECT id as value, name as text FROM #__agency  ORDER BY name");
    else
        $database->setQuery("SELECT id as value, name as text FROM #__agency  where status >=0 ORDER BY name");
    $alist = array_merge($alist, $database->loadObjectList());
    if (isset($agent->id_agency))
...
function agents_list($id, $search = "", $page = 0, $agency_id = 0) {
    ...
    $database->query();
    switch ($ss) {
        case 0: $add = " where a.status >=0 ";
            break;
        case 1: $add = " where a.status= -1 ";
            break;
    }
    if ($search != "")
        $add = " where  a.name like '%" . $search . "%'";
    $result.= "
  <table width='95%' border='0' align='center' cellpadding='0' cellspacing='0'>
    ...
    if ($agency_id > 0) {
        ...
        switch ($id) {
            case 0: $add = " where a.status >=0 and a.id_agency=" . $agency_id;
                break;
            case 1: $add = " where a.status= -1 and a.id_agency=" . $agency_id;
                break;
        }
...
//$result.=$query;
    ...
    $pp = $page - 1;
    if (1 == $page)
        $links = "<div align='center'><span class='style11'><img border=0 src='images/back-24x24.png' align='absmiddle'>&nbsp;prev&nbsp;page&nbsp;";
    else
        $links = "<div align='center'><a href='#' onclick='javascript:agent_list(" . $id . ",\"" . $search . "\"," . $pp . ");'><img border=0 src='images/back-24x24.png' align='absmiddle'></a>&nbsp;<a class='style11' href='#' onclick='javascript:agent_list(" . $id . ",\"" . $search . "\"," . $pp . ");'>prev&nbsp;page</a>&nbsp;";
    $links.= $paginator;
    if ($pages == $page)
        $links.="<span class='style11'>&nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span>";
    else
        $links.="&nbsp;<a class='style11' href='#' onclick='javascript:agent_list(" . $id . ",\"" . $search . "\"," . $np . ");'>next&nbsp;page</a>&nbsp;<a href='#' onclick='javascript:agent_list(" . $id . ",\"" . $search . "\"," . $np . ");'><img border=0 src='images/next-24x24.png' align='absmiddle'></a>";
    $agents = array();
    $database->setQuery($query);
    $agents = $database->loadObjectList();
    if (sizeof($agents) > 0)
        ...
        if ($search)
            ...
            if ($search)
                $result.="<span class='style5'>You search for:&nbsp;<b>" . $search . "</b></span>";
            else {
                if ($ss != 0)
                    $result.= "<A HREF='#' onclick='javascript:agent_list(0,\"" . $search . "\"," . $page . "," . $agency_id . ");' class='style11'>Active</A>";
                else
                    $result.= "[<B>Active</B>]";
                $result.= "&nbsp;|&nbsp;";
                if ($ss != 1)
                    $result.= "<A HREF='#' class='style11' onclick='javascript:agent_list(1,\"" . $search . "\"," . $page . "," . $agency_id . ");' >Deleted</A>";
                else
                    $result.= "[<B>Deleted</B>]";
            }
            $result.="</td></tr>";
    ...
    foreach ($agents as $agent) {
        ...
        if ("" == $agency)
            ...
            $agency = "&nbsp;&nbsp;&nbsp;<a href='#' onclick='javascript:getagencyInfo(" . $agent->id_agency . ");' class='style6'><img src='images/info-24x24.png'  align='absmiddle' border='0'></a>&nbsp;&nbsp;<a class='style34' href='#' onclick='javascript:getagencyInfo(" . $agent->id_agency . ")'>" . $agency . "</a>";
        $result.="
...
<img src='images/blog-post-edit-24x24.png'  align='absmiddle' border='0'></a>&nbsp;";
        ...
        if ($ss == 0)
            $result.="<a href='#' onclick='javascript:agent_delete(" . $agent->id . ");' title='Delete this Agent'><img src='images/trash-empty-24x24.png'  align='absmiddle' border='0'></a>";
        else {
            $result.="<a href='#' onclick='javascript:agent_restore(" . $agent->id . ");' title='Restore this Agent'><img src='images/accept-24x24.png' align='absmiddle' border='0'></a>";
...
function agent_delete($id, $mode = 0) {
    ...
    if ($mode == 1)
        $query = "delete from `#__agents` where `id`=" . $id;
//echo $query;
    $database->setQuery($query);
    $database->query();
    $objResponse = new xajaxResponse('UTF-8');
...
//$result.=$query;
    ...
    if (1 == $page)
        ...
        $links = "<div align='center'><a href='#' onclick='javascript:artist_list(" . $id . ",\"" . $search . "\"," . $pp . ");'><img border=0 src='images/back-24x24.png' align='absmiddle'></a>&nbsp;<a class='style11' href='#' onclick='javascript:artist_list(" . $id . ",\"" . $search . "\"," . $pp . ");'>prev&nbsp;page</a>&nbsp;";
    $links.= $paginator;
    if ($pages == $page)
        $links.="<span class='style11'>&nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span>";
    else
        $links.="&nbsp;<a class='style11' href='#' onclick='javascript:artist_list(" . $id . ",\"" . $search . "\"," . $np . ");'>next&nbsp;page</a>&nbsp;<a href='#' onclick='javascript:artist_list(" . $id . ",\"" . $search . "\"," . $np . ");'><img border=0 src='images/next-24x24.png' align='absmiddle'></a>";
    $artists = array();
    $database->setQuery($query);
...
";
            ...
            if ($search)
                ...
                if ($ss != 0)
                    $result.= "<A HREF='#' onclick='javascript:artist_list(0);' class='style11'>Active</A>";
                else
                    $result.= "[<B>Active</B>]";
                $result.= "&nbsp;|&nbsp;";
                if ($ss != 1)
                    $result.= "<A HREF='#' class='style11' onclick='javascript:artist_list(1);' >Deleted</A>";
                else
                    $result.= "[<B>Deleted</B>]";
            }
            $result.="</td></tr>";
...
<td width='33%' height='35' bgcolor='white' nowrap>&nbsp;";
        ...
        if ($search)
            $result.="<input type='checkbox' align='absmiddle' id='artist_" . $artist->id . "' prop='mail' boxtype='artist' email='" . $artist->email . "'>&nbsp;";
...
";
        if ($ss == 0)
            $result.="<a href='#' onclick='javascript:artist_delete(" . $artist->id . ");' title='Delete this Artist'><img src='images/trash-empty-24x24.png'  align='absmiddle' border='0'></a>";
        else {
...
function editartistInfo($artist_id) {
    ...
    if (isset($artist->id_agency))
        $agnts = $artist->id_agency;
    else
        $agnts = 0;
...
<TR><TD>Artist</TD><TD><INPUT TYPE='text' NAME='name' required='1'  value='";
    ...
    if (isset($artist->contact_person))
        $result.=$artist->contact_person;
    $result.="'></TD></TR>";
    $alist = array();
    $alist[] = mosHTML::makeOption('0', 'undefined ...', 'value', 'text');
    if (isset($artist->id_agency))
        $database->setQuery("SELECT id as value, name as text from #__agents  where id_agency=" . $artist->id_agency . " ORDER BY name");
    else
        $database->setQuery("SELECT id as value, name as text from #__agents  where status >=0 ORDER BY name");
    $alist = array_merge($alist, $database->loadObjectList());
    if (isset($artist->id_agent))
        $agnts = $artist->id_agent;
    else
        $agnts = 0;
    ...
    if (isset($artist->phone1))
        $result.=$artist->phone1;
    $result.="'></TD></TR><TR><TD>Cell phone</TD><TD><INPUT TYPE='text' id='phone2'NAME='phone2' value='";
    if (isset($artist->phone2))
        $result.=$artist->phone2;
...
//echo $query;
    if ($mode == 1)
        $query = "delete from `#__artists` where `id`=" . $itemId;
    $database->setQuery($query);
    $database->query();
    $objResponse = new xajaxResponse('UTF-8');
    $objResponse->addAssign('report_div', 'innerHTML', artists_list(0));
...
function inquiry_delete($itemId, $mode = 0) {
    ...
    if ($mode == 1)
        $query = "delete from `#__inquiries` where `id`=" . $itemId;
    $database->setQuery($query);
    $database->query();
    $objResponse = new xajaxResponse('UTF-8');
...
<TR>
    ...
    if (isset($inq->town))
        ...
        $result.="";
    if (isset($promoter->town))
        $result.=$promoter->town;
    else
        $result.="";
    $result.="'></TD>
</TR>
<TR>
    ...
    if (isset($inq->venue_date))
        $result.=substr_replace($inq->venue_date, '', 16, 16);
    else
        $result.="";
    ...
    $alist = array_merge($alist, $database->loadObjectList());
    if (isset($inq->id_promoter))
        $agnts = $inq->id_promoter;
    else
        $agnts = 0;
    if ($promoter->id > 0)
        $agnts = $promoter->id;
    ...
    if (isset($inq->company))
        $result.=$inq->company;
    if (isset($promoter->company))
...
<div id='check_results' name='check_results'></div>
    ...
    if (isset($inq->phone1))
        ...
        $result.="";
    if (isset($promoter->phone1))
        $result.=$promoter->phone1;
    else
        $result.="";
    $result.="'></TD>
</TR>
<TR>
    ...
    if (isset($inq->contact_person))
        $result.=$inq->contact_person;
    else
        $result.="";
    if (isset($promoter->contact_person))
        $result.=$promoter->contact_person;
    else
        $result.="";
    $result.="'></TD>
    ...
    if (isset($inq->phone2))
        ...
        $result.="";
    if (isset($promoter->phone2))
        $result.=$promoter->phone2;
    else
        $result.="";
    $result.="'></TD>
</TR>
<TR>
    ...
    if (isset($inq->address1))
        $result.=$inq->address1;
    else
        $result.="";
    if (isset($promoter->street_addr1))
        $result.=$promoter->street_addr1;
    else
        $result.="";
    $result.="'></TD>
    ...
    if (isset($inq->email))
        ...
        $result.="";
    if (isset($promoter->email))
        $result.=$promoter->email;
    else
        $result.="";
    $result.="'></TD>
</TR>
<TR>
    ...
    if (isset($inq->address2))
        $result.=$inq->address2;
    else
        $result.="";
    if (isset($promoter->street_addr2))
        $result.=$promoter->street_addr2;
    else
        $result.="";
    $result.="'></TD>
    ...
    if (isset($inq->www))
        ...
        $result.="";
    if (isset($promoter->website))
        $result.=$promoter->website;
    else
        $result.="";
    $result.="'></TD>
</TR>
<TR>
    ...
    if (isset($inq->city_code))
        $result.=$inq->city_code;
    else
        $result.="";
    if (isset($promoter->city_code))
        $result.=$promoter->city_code;
    else
        $result.="";
    $result.="'></TD>
	<TD>&nbsp;</TD><TD>&nbsp;</TD>
...
<TR>
    ...
    if (isset($inq->comments))
        $result.=$inq->comments;
    else
        $result.="";
    if (isset($promoter->comments))
        $result.=$promoter->comments;
    else
        $result.="";
    $result.="</TEXTAREA></TD>
</TR>
    ...
    if (isset($inq->artist_fee))
        $result.=$inq->artist_fee;
    else
        $result.="0";
    ...
    if (isset($inq->admin_exp))
        $result.=$inq->admin_exp;
    else
        $result.="0";
    $result.="' size='10'  onblur='javascript:calculate_fee();'>
                      </td>
    ...
    if (isset($inq->production_exp))
        $result.=$inq->production_exp;
    else
        $result.="0";
    $result.="' size='10'  onblur='javascript:calculate_fee();'>
                      </td>
    ...
    if (isset($inq->other_exp))
        $result.=$inq->other_exp;
    else
        $result.="0";
    $result.="' size='10'  onblur='javascript:calculate_fee();'>
                      </td></tr>
    ...
    if (isset($inq->travel_exp))
        $result.=$inq->travel_exp;
    else
        $result.="0";
    ...
    if (isset($inq->total_exp))
        $result.=$inq->total_exp;
    else
        $result.="0";
    $result.="' size='10' ></td>
                    </tr><tr>
...
function save_inquiry($fdatas) {
    ...
    if (($fdatas['artist_fee'] == 'undefined') || ($fdatas['artist_fee'] == ''))
        $fdatas['artist_fee'] = 0;
    if (($fdatas['admin_exp'] == 'undefined') || ($fdatas['admin_exp'] == ''))
        $fdatas['admin_exp'] = 0;
    if (($fdatas['production_exp'] == 'undefined') || ($fdatas['production_exp'] == ''))
        $fdatas['production_exp'] = 0;
    if (($fdatas['other_exp'] == 'undefined') || ($fdatas['other_exp'] == ''))
        $fdatas['other_exp'] = 0;
    if (($fdatas['travel_exp'] == 'undefined') || ($fdatas['travel_exp'] == ''))
        $fdatas['travel_exp'] = 0;
    if (($fdatas['total_exp'] == 'undefined') || ($fdatas['total_exp'] == ''))
        $fdatas['total_exp'] = 0;
...
`currency`=\"" . $fdatas['currency'] . "\",
 ";
        ...
        if (isset($fdatas['id_promoter']))
            $query.=" `id_promoter`=" . $fdatas['id_promoter'] . ", ";
        $query.=" `www`=\"" . $fdatas['www'] . "\",
`city_code`=\"" . $fdatas['city_code'] . "\",
`comments`=\"" . $fdatas['comments'] . "\",";

        if (trim($fdatas['venue_date']) == '')
            $query.=" venue_date=NULL, ";
        else
            $query.="venue_date=\"" . $fdatas['venue_date'] . "\",";
        $query.="
`status`=1,
...
function inquirys_list($id, $search = "", $page = 0) {
    ...
    switch ($ss) {
        case 0: $add = " and i.status >=0 ";
            break;
        case 1: $add = " and i.status >=0 ";
            break;
        case 2: $add = " and i.status =-1 ";
            break;
    }
    if ($search != "")
        $add = " and ( i.company like '%" . $search . "%' or  i.contact_person like '%" . $search . "%')";
    $result.= "
  <table width='95%'  border='0' align='center' cellpadding='0' cellspacing='0'>
      <tr>
...
ORDER BY i.status asc, i.id desc" . $limits;
//$result.=$query;
    $np = $page + 1;
    $pp = $page - 1;
    if (1 == $page)
        ...
        $links = "<div align='center'><a href='#' onclick='javascript:inquiry_list(" . $id . ",\"" . $search . "\"," . $pp . ");'><img border=0 src='images/back-24x24.png' align='absmiddle'></a>&nbsp;<a class='style11' href='#' onclick='javascript:inquiry_list(" . $id . ",\"" . $search . "\"," . $pp . ");'>prev&nbsp;page</a>&nbsp;";
    $links.= $paginator;
    if ($pages == $page)
        $links.="<span class='style11'>&nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span>";
    else
        $links.="&nbsp;<a class='style11' href='#' onclick='javascript:inquiry_list(" . $id . ",\"" . $search . "\"," . $np . ");'>next&nbsp;page</a>&nbsp;<a href='#' onclick='javascript:inquiry_list(" . $id . ",\"" . $search . "\"," . $np . ");'><img border=0 src='images/next-24x24.png' align='absmiddle'></a>";
            ...
            $result.= "<table width='95%' border='0' align='center' cellpadding='0' cellspacing='1' bgcolor='#999999'>
              <tr>
                ...
                <span class='style5'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;You search for&nbsp;<b>" . $search . "</b>,&nbsp;but nobody found...</span></td></TR>
                  <tr>
                    <td height='35' bgcolor='#FFFFFF'>&nbsp;&nbsp;
                    <a href='#' onclick='javascript:editinquiryInfo(0);'><img src='images/comment-add-24x24.png'   align='absmiddle' border=0></a>&nbsp;&nbsp;<a href='#' onclick='javascript:editinquiryInfo(0);'  class='style11'>Add New Iquiry</a></td>
                    <td height='35' colspan='2' bgcolor='#FFFFFF'>&nbsp;</td>
                  </tr>
                </table></td>
                  </tr>
...
";
            ...
            if ($search)
                ...
                if ($ss != 0)
                    $result.= "<A HREF='#' onclick='javascript:inquiry_list(0);' class='style11'>Active</A>";
                else
                    $result.= "[<B>Active</B>]";
                $result.= "&nbsp;|&nbsp;";
                if ($ss != 1)
                    $result.= "<A HREF='#' class='style11' onclick='javascript:inquiry_list(2);' >Deleted</A>";
                else
                    $result.= "[<B>Deleted</B>]";
            }
            $result.="</td></tr>";
...
<tr><td bgcolor='#999999' class='style4'><table width='100%' border='0' align='center' cellpadding='0' cellspacing='1'>";
    ...
    if ($search)
        $result.="<tr><td height='36' colspan='4' bgcolor='#FFFFFF'>&nbsp;&nbsp;<span class='style5'>Inquires</span>&nbsp;&nbsp;&nbsp;<span class='style5'>You search for:&nbsp;<b>" . $search . "</b></span></td></tr>";

    if ($ss != 0)
        $act_del.= "<A HREF='#' onclick='javascript:inquiry_list(0);' class='style11'>Active</A>";
    else
        $act_del.= "[<B>Active</B>]";
    $act_del.= "&nbsp;|&nbsp;";
    if ($ss != 1)
        $act_del.= "<A HREF='#' class='style11' onclick='javascript:inquiry_list(2);' >Deleted</A>";
    else
        $act_del.= "[<B>Deleted</B>]";
    $result.="";
    ...
    foreach ($inquirys as $inquiry) {
        ...
        if ($inquiry->ddates > "")
            $inquiry->ddates = "<br/>" . $inquiry->ddates;
        if ($inquiry->status == 0) {
            if (!$head1)
...
";
            ...
            if ($ss == 0) {
                $result.="<a href='#' onclick='javascript:inquiry_delete(" . $inquiry->id . ");' title='Delete this Inquiry'>
          ...
          <a class='style34' href='#' onclick='javascript:viewInquiry(" . $inquiry->id . ")'>" . $inquiry->promoter_name . "</a>";
            if ($inquiry->id_promoter == 0) {
                $result.= "&nbsp;&nbsp;&nbsp;<span id='i" . $inquiry->id . "' name='i" . $inquiry->id . "' >
...
";
            ...
            if ($ss == 0)
                $result.="<a href='#' onclick='javascript:inquiry_delete(" . $inquiry->id . ");' title='Delete this Inquiry'><img src='images/trash-empty-24x24.png'  align='absmiddle' border='0'></a>";
            else {
                $result.="<a href='#' onclick='javascript:inquiry_restore(" . $inquiry->id . ");' title='Restore this Inquiry'><img src='images/accept-24x24.png' align='absmiddle' border='0'></a>";
...
<a class='style6' href='#' onclick='add_promoter(" . $inquiry->id . ")' title='add customer to DB'><IMG SRC='images/user-add-24x24.png' align='absmiddle' BORDER='0' ALT='Add customer to DB'></a></span>";
            ...
            $result.="</td><td width='30%' bgcolor='FFCCCC'><div align='center'><span class='style6'>" . $inquiry->artist_name . "</span></div></td>
                    <td width='12%' bgcolor='FFCCCC'><div align='center'>" . $vd[0] . $inquiry->ddates . "</div></td>
                    <td width='10%' bgcolor='FFCCCC' id='box_" . $inquiry->id . "'><div align='center'>" . $inquiry->id . "&nbsp;&nbsp;<input type='checkbox' id='inq_" . $inquiry->id . "' onchange='get_set_marker(" . $inquiry->id . ");' /></div></td>
                  </tr>
";
        }
    }
...
function promoters_list($id, $search = "", $page = 0, $weeknum = "", $country = 0, $town = "", $priority = 0) {
    ...
    $add = " where 1=1 ";
    if ($search != "")
        $add .=" and (upper(a.name) like '%" . mb_strtoupper(mb_convert_encoding($search, 'utf8', 'UTF-8')) . "%' or  upper(a.name) like '%" . mb_strtoupper($search) . "%'  or  upper(a.contact_person) like '%" . mb_strtoupper(htmlentities($search, ENT_QUOTES, "UTF-8")) . "%' or  upper(a.contact_person) like '%" . mb_strtoupper($search) . "%' or  upper(a.email) like '%" . mb_strtoupper($search) . "%' ) ";
    if ($weeknum != "")
    ...
    switch ($priority) {
        case 1: $add.="and a.priority = 0";
            break;
        case 2: $add.="and a.priority = 1";
            break;
        case 3: $add.="and a.priority = 2";
            break;
        case 4: $add.="and a.priority = 3";
            break;
        default: break;
    }
    if ($ss)
...
order by a.priority desc, a.name asc" . $limits;
    $np = $page + 1;
    $pp = $page - 1;
    if (1 == $page)
        $links = "<div align='center'><span class='style11'><img border=0 src='images/back-24x24.png' align='absmiddle'>&nbsp;prev&nbsp;page&nbsp;";
    else
        $links = "<div align='center'><a href='#' onclick='javascript:promoter_list(" . $id . ",\"" . $search . "\"," . $pp . ",\"" . $wnn . "\",\"" . $country . "\");'><img border=0 src='images/back-24x24.png' align='absmiddle'></a>&nbsp;<a class='style11' href='#' onclick='javascript:promoter_list(" . $id . ",\"" . $search . "\"," . $pp . ",\"" . $wnn . "\",\"" . $country . "\",\"" . $town . "\");'>prev&nbsp;page</a>&nbsp;";
    $links.= $paginator;
    if ($pages == $page)
        $links.="<span class='style11'>&nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span>";
    else
        $links.="&nbsp;<a class='style11' href='#' onclick='javascript:promoter_list(" . $id . ",\"" . $search . "\"," . $np . ",\"" . $wnn . "\",\"" . $country . "\");'>next&nbsp;page</a>&nbsp;<a href='#' onclick='javascript:promoter_list(" . $id . ",\"" . $search . "\"," . $np . ",\"" . $wnn . "\",\"" . $country . "\",\"" . $town . "\");'><img border=0 src='images/next-24x24.png' align='absmiddle'></a>";
    ...
    $r1 = $r2 = '';
    if ((sizeof($country) > 0) && ($country != 0))
        $r1 = ' in <b>' . $country_name . '</b> ';
    if ($weeknum != "")
        $r2 = ' in <b>' . $weeknum . '</b> week ';
    if ($town != "")
        $r3 = ' in <b>' . $town . '</b>';
    $STAT = "";
    if ($search || $weeknum || $country || $town)
        $STAT = "<span class='style5'>You search for:&nbsp;<b>" . $search . "</b>  " . $r1 . $r2 . $r3 . "</span>";
    else {
        if ($ss != 0)
            $STAT.= "<A HREF='#' onclick='javascript:promoter_list(0);' class='style11'>Active</A>";
        else
            $STAT.= "[<B>Active</B>]";
        $STAT.= "&nbsp;|&nbsp;";
        if ($ss != 1)
            $STAT.= "<A HREF='#' class='style11' onclick='javascript:promoter_list(1);' >Deleted</A>";
        else
            $STAT.= "[<B>Deleted</B>]";
    }
    ...
    foreach ($promoters as $promoter) {
        if ($promoter->priority == 3)
            $to_priority = 0;
        else
            $to_priority = 1 + $promoter->priority;
        switch ($promoter->priority) {
            case 1: $img = 'square-red.png';
                break;
            case 2: $img = 'square-yellow.png';
                break;
            case 3: $img = 'square-green.png';
                break;
            default: $img = 'square-white.png';
                break;
        }
        ($promoter->phone2) ? $cell = " phone=\"" . $promoter->phone2 . "\" " : $cell = "";
        if ($promoter->checked > 0)
            $chk = " checked ";
        else
            $chk = "";
    ...
    if (sizeof($promoters) > 0)
        $html.=$ft->FastPrint('ITEMS', true);
    $ft->assign(array(
        'LINKS' => $links));
    $ft->parse('FOOT', 'foot');
...
//}
    ...
    $add = " where 1=1 ";
    if ($search != "")
        $add .=" and ( upper(a.name) like '%" . mb_strtoupper(mb_convert_encoding($search, 'utf8', 'UTF-8')) . "%' or  upper(a.name) like '%" . mb_strtoupper($search) . "%'  or  upper(a.contact_person) like '%" . mb_strtoupper(htmlentities($search, ENT_QUOTES, "UTF-8")) . "%' or  upper(a.contact_person) like '%" . mb_strtoupper($search) . "%' ) ";
    if ($weeknum != "")
        $add .=" and a.weeknum in (" . $weeknum . ") ";
    if ($town != "")
        $add .=" and upper(a.location) like '%" . mb_strtoupper(mb_convert_encoding($town, 'utf8', 'UTF-8')) . "%' ";
...
&nbsp;<a href='#' onclick='javascript:checkbox_reader();'  class='style11'>Send emails to checked</a>";
            ...
            if (($search) || ($weeknum) || ($town) || ($country > 0))
                $result.="<span class='style5'>You search for:&nbsp;<b>" . $search . "</b></span>";
            else {
                if ($ss != 0)
                    $result.= "<A HREF='#' onclick='javascript:promoter_list(0);' class='style11'>Active</A>";
                else
                    $result.= "[<B>Active</B>]";
                $result.= "&nbsp;|&nbsp;";
                if ($ss != 1)
                    $result.= "<A HREF='#' class='style11' onclick='javascript:promoter_list(1);' >Deleted</A>";
                else
                    $result.= "[<B>Deleted</B>]";
            }
            $result.="</td></tr>";
...
&nbsp;<a href='#' onclick='javascript:checkbox_reader(\"promoter\");'  class='style11'>Send emails to checked</a>";
    ...
    if ($town != "")
        $r3 = ' in <b>' . $town . '</b>';
    if ($search || $weeknum || $country || $town)
        $result.="<span class='style5'>You search for:&nbsp;<b>" . $search . "</b>  " . $r1 . $r2 . $r3 . "</span>";
    else {
        if ($ss != 0)
            $result.= "<A HREF='#' onclick='javascript:promoter_list(0);' class='style11'>Active</A>";
        else
...
";
        ...
        if ($ss == 0)
            $result.="<a href='#' onclick='javascript:promoter_delete(" . $promoter->id . ");' title='Delete this Promoter'><img src='images/trash-empty-24x24.png'  align='absmiddle' border='0'></a>";
        else {
            $result.="<a href='#' onclick='javascript:promoter_restore(" . $promoter->id . ");' title='Restore this Promoter'><img src='images/accept-24x24.png' align='absmiddle' border='0'></a>";
...
//}
    ...
    $add = " where 1=1 ";
    if ($search != "")
        $add .=" and ( upper(a.name) like '%" . mb_strtoupper($search) . "%' or  upper(a.contact_person) like '%" . mb_strtoupper($search) . "%' ) ";
    if ($town != "")
        $add .=" and ( upper(a.town) like '%" . mb_strtoupper($town) . "%' ) ";
    if ($weeknum != "")
        $add .=" and a.weeknum in (" . $weeknum . ") ";
    if ((sizeof($country) > 0) && ($country != 0)) {
        $add .=" and a.country in (" . $country . ") ";
...
function promoter_delete($promoter_id, $mode = 0) {
    ...
    if ($mode == 1)
        $query = "delete from `#__promoters` where `id`=" . $promoter_id;
    $database->setQuery($query);
    $database->query();
    $objResponse = new xajaxResponse('UTF-8');
    $objResponse->addAssign('report_div', 'innerHTML', promoters_list(0));
...
<TR><TH colspan=2 class='h2'>";
    if ($promoter_id > 0)
        $result.="Promoter&nbsp;N&deg;<b>" . $promoter_id . "</b>&nbsp;information";
    else
        $result.="New Promoter information";
    $result.="</TH>
</TR><TR>	<TD valign='top'  class='h1'>";
...
&nbsp;</div>";
    ...
    if (isset($promoter->weeknum))
        $result.=$promoter->weeknum;
    else
        $result.="00";
    ...
    if (isset($promoter->location))
        $result.=$promoter->location;
    else
        $result.="";
    $result.="'></TD></TR><TR><TD>Country</TD><TD>";
    $clist = array();
...
";
            ...
            if ($search)
                ...
                if ($ss != 0)
                    $result.= "<A HREF='#' onclick='javascript:sound_list(0);' class='style11'>Active</A>";
                else
                    $result.= "[<B>Active</B>]";
                $result.= "&nbsp;|&nbsp;";
                if ($ss != 1)
                    $result.= "<A HREF='#' class='style11' onclick='javascript:sound_list(1);' >Deleted</A>";
                else
                    $result.= "[<B>Deleted</B>]";
            }
            $result.="</td></tr>";
...
";
        ...
        if ($ss == 0)
            $result.="<a href='#' onclick='javascript:sound_delete(" . $sound->id . ");' title='Delete this sound'><img src='images/trash-empty-24x24.png'  align='absmiddle' border='0'></a>";
        else
            $result.="<a href='#' onclick='javascript:sound_restore(" . $sound->id . ");' title='Restore this sound'><img src='images/accept-24x24.png' align='absmiddle' border='0'></a>";
...
<TR><TH colspan=2 class='h2'>Sound company&nbsp;info&nbsp;№&nbsp;" . $id . "</TH>
        ...
        if (isset($sound_companie->name))
            $result.=$sound_companie->name;
        else
            $result.="";
        $result.="</TD></TR><TR><TD>Contact person</TD><TD>";
        if (isset($sound_companie->contact_person))
            $result.=$sound_companie->contact_person;
        else
            $result.="";
        $result.="</TD></TR><TR><TD>Street address 1</TD><TD>";
        if (isset($sound_companie->street_addr1))
            $result.=$sound_companie->street_addr1;
        else
            $result.="";
        $result.="</TD></TR><TR><TD>Street address 2</TD><TD>";
        if (isset($sound_companie->street_addr2))
            $result.=$sound_companie->street_addr2;
        else
            $result.="";
        $result.="</TD></TR><TR><TD>City code (ZIP)</TD><TD>";
        if (isset($sound_companie->city_code))
            $result.=$sound_companie->city_code;
        else
            $result.="";
        $result.="</TD></TR><TR><TD>Town</TD><TD>";
        if (isset($sound_companie->town))
            $result.=$sound_companie->town;
        else
            $result.="";
        $result.="</TD></TR><TR><TD>Country</TD><TD>";
        ...
        if (isset($sound_companie->phone1))
            $result.=$sound_companie->phone1;
        else
            $result.="";
        $result.="</TD></TR><TR><TD>Phone 2</TD><TD>";
        if (isset($sound_companie->phone2))
            $result.=$sound_companie->phone2;
        else
            $result.="";
        $result.="</TD></TR><TR><TD>e-mail</TD><TD>";
        if (isset($sound_companie->email))
            $result.="<a href='mailto:" . $sound_companie->email . "'>" . $sound_companie->email . "</a>";
        else
            $result.="";
        $result.="</TD></TR><TR><TD>Website</TD><TD>";
        if (isset($sound_companie->website))
            $result.="<a href='http://" . $sound_companie->website . "' target='_blank'>" . $sound_companie->website . "</a>";
        else
            $result.="";
        $result.="</TD></TR>";
...
<TR><TH colspan=2 class='h2'>Sound company&nbsp;info&nbsp;№&nbsp;" . $id . "</TH>
    ...
    if (isset($sound_companie->id))
        $result.=$sound_companie->id;
    else
        $result.="0";
    ...
    if (isset($sound_companie->id_country))
        $cnt = $sound_companie->id_country;
    else
        $cnt = 1;
    ...
    $result.="'></TD></TR><TR><TD>e-mail</TD><TD><INPUT TYPE='text' NAME='email' value='";
    if (isset($sound_companie->email))
        $result.=$sound_companie->email;
    else
        $result.="";
    $result.="'></TD></TR><TR><TD>Website</TD><TD><INPUT TYPE='text' NAME='website' value='";
    if (isset($sound_companie->website))
        $result.=$sound_companie->website;
    else
        $result.="";
    $result.="'></TD></TR>";
    $result.="</TD></TR></TABLE></TD></TR><tr><td align='center' colspan=2>";
...
//$es=email_selectors($to_type,$to_id,$data);
    if ($message_id == 0)
        $subm = "<div id='sendbutton'><INPUT TYPE='submit' value='Save message template'></div><div id='waitsend' style='display:none;'><img src='images/wait_big.gif' border=0 vspace=3 hspace=3></div>";
    else
        $subm = '';
...
//$result=mass_mail_draw_menu($id);
    switch ($id) {
        case 0: $result.=mass_mail_msg_list();
            break;
        case 1: $result.=mass_mail_mailing_list($item);
            break;
        case 2: $result.=mass_mail_mailing_cont_list($item, $page);
            break;
        case 3: $result.=mass_mail_queque_list($page);
            break;
        default: $result.="";
            break;
    }
...
function user_list($id) {
    ...
    switch ($ss) {
        case 0: $add = " where status >0 ";
            break;
        case 1: $add = " where status <1 ";
            break;
    }
    ...
    if ($id != 0)
        $result.= "<A HREF='#' onclick='javascript:users_list(0);'>Active</A>";
    else
        $result.= "[<B>Active</B>]";
    $result.= "&nbsp;|&nbsp;";
    if ($id != 1)
        $result.= "<A HREF='#' onclick='javascript:users_list(1);'>Blocked</A>";
    else
        $result.= "[<B>Blocked</B>]";
    $result.= "</TD></TR></TABLE>";
    ...
    if (sizeof($users) > 0)
        $result.= "<div style='z-index:3;'><TABLE class='tt'><TR><TH>id</TH><TH>Action</TH><TH>Login</TH><TH>User name</TH><TH>Status</TH><TH>Last login</TH></TR>";
    else
        $result.= "<TABLE class='tt'><TR><TH>Nobody here...</TH></TR></TABLE>";
...
function viewSchedules($id_artist, $yr = 2012, $date_from = "", $date_to = "", $week = 0) {
    ...
    if ((isset($date_from)) && (((!isset($date_to)) || ($date_to == "") || ($date_to == "undefined"))))
        $date_to = $date_from;
    if ((!isset($date_from)) || ($date_from == "") || ($date_from == "undefined"))
        $date_from = date("Y-m-d");
    if ((!isset($date_to)) || ($date_to == "") || ($date_to == "undefined"))
        $date_to = date("Y-m-d");


    list($year_e, $month_e, $day_e) = explode("-", $date_to);
    list($year_b, $month_b, $day_b) = explode("-", $date_from);
    if ($year_b * 1 > $year_e * 1) {
        $o = $year_b * 1;
        $year_b = $year_e * 1;
        $year_e = $o;
    }
    if (($year_b == $year_e) && ($month_b > $month_e)) {
        $o = $month_b * 1;
        $month_b = $month_e * 1;
        $month_e = $o;
    }
...
//session_register('id_artist');
    ...
    $result.="</td></tr></table>";
    $i = 0;
    $result.="<table align='center' cellpadding=10><tr>";
...
//$result="===$id===$pid===\n<br />";
    ...
    if (sizeof($perfs) > 0)
        foreach ($perfs as $perf) {
            ...
            if ($perf->freeday > 0)
                $FD = "<div style='float:right'><b>ONE DAY OFF</b></div>";
            else
                $FD = "";

            if (strlen($perf->hotel_link) > 20)
                $perf->hotel_link = "<a href='" . $perf->hotel_link . "' target='_blank'>" . substr($perf->hotel_link, 0, 20) . "...</a>";
            $result.="
      <table width='95%' border='0' align='center' cellpadding='0' cellspacing='1' bgcolor='#999999'>
        <tr>
          ...
          <td bgcolor='#999999' class='style4'><table width='100%' border='0' align='center' cellpadding='1' cellspacing='1'>
              ...
              <tr>
                <td height='36' bgcolor='#FFFFFF' class='style17'><div align='left'><span class='style17'>Promoter</span></div></td>
...
//session_register('id_artist');
    ...
    if (isset($perf->id))
        ...
        $id_perf = 0;
    if (isset($perf->perf_duration))
        $PD = $perf->perf_duration;
    else
        $PD = $cont->lenght_of;
    if (isset($perf->concert_start))
        $CS = substr_replace($perf->concert_start, '', 5, 5);
    else
        $CS = substr_replace($cont->concert_start, '', 5, 5);
    ...
    $promoters = $database->loadObjectList();
    if (isset($perf->id_promoter))
        $cnt = $perf->id_promoter;
    else
        $cnt = $cont->id_promoter;;
    $PL = mosHTML::selectList($promoters, 'id_promotere', ' DISABLED id="id_promotere" ', 'id', 'name', $cnt);
    if (isset($perf->capacity))
        $CP = $perf->capacity;
    else
        $CP = $cont->capacity;
    if (isset($perf->venue))
    ...
    if (isset($perf->city))
        ...
        $CT = $cont->town;
    if (isset($perf->date_of))
        $DA = substr_replace($perf->date_of, '', 10, 10);
    else
        $DA = substr_replace($cont->concert_date, '', 10, 10);
    if (isset($perf->contract_id))
        $CI = $perf->contract_id;
    else
        $CI = $id_contract;
    if (isset($perf->onstage))
        $OS = $perf->onstage;
    else
        $OS = $cont->concert_start;
    if (isset($perf->getintime))
        $GI = $perf->getintime;
    else
        $GI = $cont->acc_to_stage;
    if (isset($perf->dinner))
        $DI = $perf->dinner;
    else
        $DI = $cont->dinner;
    if ($perf->freeday == 1)
...
`production`=\"" . $fdatas['production'] . "\",";
        if (trim($fdatas['pressconf']) == '')
            $query.=" pressconf=NULL, ";
        else
            $query.="`pressconf`=\"" . $fdatas['pressconf'] . "\",";
        if (trim($fdatas['getintime']) == '')
        ...
        if (trim($fdatas['dinner']) == '')
            ...
            $query.="dinner=\"" . $fdatas['dinner'] . "\",";
        if (trim($fdatas['doorsopen']) == '')
            $query.="doorsopen=NULL,";
        else
            $query.="doorsopen=\"" . $fdatas['doorsopen'] . "\",";
        if (trim($fdatas['concert_start']) == '')
            $query.="concert_start=NULL,";
        else
            $query.="concert_start=\"" . $fdatas['concert_start'] . "\",";
        if (trim($fdatas['onstage']) == '')
            $query.="onstage=NULL,";
        else
            $query.="onstage=\"" . $fdatas['onstage'] . "\",";
        if (trim($fdatas['contract_id']) == '')
            $query.="contract_id=NULL,";
        else
            $query.="contract_id=\"" . $fdatas['contract_id'] . "\",";
        $query.="
...
\"" . $fdatas['date_of'] . "\",\"" . $fdatas['country'] . "\",\"" . $fdatas['city'] . "\",\"" . $fdatas['venue'] . "\",\"" . $fdatas['venue_street'] . "\",\"" . $fdatas['venue_phone'] . "\",\"" . $fdatas['venue_email'] . "\",\"" . $fdatas['venue_link'] . "\",\"" . $fdatas['capacity'] . "\"," . $fdatas['id_promoter'] . ",	" . $fdatas['id_artist'] . ",\"" . $fdatas['hotel'] . "\",";
        ...
        if (trim($fdatas['pressconf']) == '')
            ...
            $query.="\"" . $fdatas['pressconf'] . "\",";
        if (trim($fdatas['getintime']) == '')
            $query.="NULL,";
        else
            $query.="\"" . $fdatas['getintime'] . "\",";
        if (trim($fdatas['soundcheck']) == '')
            $query.="NULL,";
        else
            $query.="\"" . $fdatas['soundcheck'] . "\",";
        if (trim($fdatas['dinner']) == '')
            $query.="NULL,";
        else
            $query.="\"" . $fdatas['dinner'] . "\",";
        if (trim($fdatas['doorsopen']) == '')
            ...
            $query.="\"" . $fdatas['doorsopen'] . "\",";
        if (trim($fdatas['concert_start']) == '')
            $query.="NULL,";
        else
            $query.="\"" . $fdatas['concert_start'] . "\",";
        if (trim($fdatas['onstage']) == '')
            $query.="NULL,";
        else
            $query.="\"" . $fdatas['onstage'] . "\",";
        if (trim($fdatas['contract_id']) == '')
            $query.="NULL,";
        else
            $query.="\"" . $fdatas['contract_id'] . "\",";
...
function add_deparr($id, $did = 0) {
    ...
    if (sizeof($dep_arrs) > 0)
        foreach ($dep_arrs as $dep_arr) {
...
<TR></TR>
<TR><TD rowspan='2'><INPUT TYPE='text' NAME='date_of' id='date_of' value='" . $date_of . "'> &nbsp;
...
function add_contract2($formdata, $id_inquiry = 0, $id_contract = 0) {
    ...
    if ($id_contract == 0)
        $database->setQuery("SELECT * from #__artists  where id=" . $formdata['id_artist']);
    else
        $database->setQuery("SELECT * from #__artists  where id=" . $c->id_artist);
    $alists = $database->loadObjectList();

    if ($id_contract == 0)
        $database->setQuery("SELECT * from #__promoters  where id=" . $formdata['id_promoter']);
    else
        $database->setQuery("SELECT * from #__promoters  where id=" . $c->id_promoter);
    if (isset($formdata['venue_date']))
        $v = explode(" ", $formdata['venue_date']);$venue_date = $v[0];
...
<form name='add_cont2' id='add_cont2' method='POST' onsubmit='javascript:prepare_dates();check_this_form(this);' ACTION=\"javascript:";
    if ($has_perf)
        $result.="add_contract4";
    else
        $result.="add_contract3";
...
<table width='95%' border='0' align='center' cellpadding='0' cellspacing='1' bgcolor='#999999'>
    ...
    if (isset($c->contract_date))
        $result.=$c->contract_date;
    else
        $result.=$today;
    $result.="'/>&nbsp;
    ...
    if (isset($c->id_promoter))
        $result.=$c->id_promoter;
    else
        $result.=$formdata['id_promoter'];
    $result.="' />
    ...
    if (isset($c->contract_no))
        $result.=" value='" . $c->contract_no . "' ";
    else
        $result.=" value='" . $mid . "/" . date("Y") . "' ";
    $result.=" /></div></td>
                       <td width='18%' height='36' bgcolor='#FFFFFF' class='style17'><div align='left'>
                      Contact person
                      </div></td>
    ...
    if (isset($c->contact_person))
        $result.=$c->contact_person;
    else
        $result.=$plist->contact_person;
    ...
    if (isset($c->id_artist))
        $result.=$c->id_artist;
    else
        $result.=$formdata['id_artist'];
    $result.="' />
    ...
    if ($id_contract == 0)
        $rr = get_inq_dated($i->id);
    else
        $rr = get_cont_dated($id_contract);
    if ($venue_date > 0) {
        $result.=$venue_date;
    } else {
        if (isset($c->concert_date))
            $result.=$c->concert_date;
    }
    $result.="' />";
    ...
    if (isset($c->address))
        $result.=$c->address; else {
        $result.=$plist->street_addr1 . "&nbsp;" . $plist->street_addr2;
    }
    $result.="' size=30 /></div></td>
                    </tr>
                    <tr>
    ...
    if (isset($c->art_of_perf))
        $result.=$c->art_of_perf;
    $result.="' size='30'>
                      </div></td>
    ...
    if (isset($c->acc_to_stage))
        $result.=$c->acc_to_stage;
    $result.="' size='30'></div></td>
                      <td width='18%' height='36' bgcolor='#FFFFFF' class='style17'><div align='left'>
                          Sound check
    ...
    if (isset($c->sound_check))
        $result.=$c->sound_check;
    $result.="' size='30'></div></td>
                    </tr>
                    <tr>
                      <td bgcolor='#FFFFFF' class='style17'><div align='left'>
    ...
    if (isset($c->concert_start))
        $result.=$c->concert_start;
    $result.="' size='30'></div></td>
                    </tr>
                    <tr>
                      <td bgcolor='#FFFFFF' class='style17'><div align='left'>
              ...
              <tr>
                <td bgcolor='#999999' class='style4'><table width='100%' border='0' align='center' cellpadding='3' cellspacing='1'>
                    <tr>
                      ...
                      <td width='29%' bgcolor='#FFFFFF' class='style17'><div align='left' class='style34'>
                      <input name='artist_fee' id='artist_fee' required  value='";
    if (isset($c->artist_fee))
        $result.=intval($c->artist_fee);
    else
        $result.="";
    ...
    $result.="' size='30' onblur='javascript:calculate_fee();'>
                      ...
                      <td bgcolor='#FFFFFF' class='style17'><div align='left' class='style34'>
                      <input name='admin_exp' id='admin_exp' required  value='";
    if (isset($c->admin_exp))
        $result.=intval($c->admin_exp);
    else
        $result.="";
    ...
    $result.="' size='30'  onblur='javascript:calculate_fee();' >
                    ...
                    <tr>
                      ...
                      <td bgcolor='#FFFFFF' class='style17'><div align='left' class='style34'>
                      <input name='production_exp' id='production_exp' required  value='";
    if (isset($c->production_exp))
        $result.=intval($c->production_exp);
    else
        $result.="";
    ...
    $result.="' size='30'  onblur='javascript:calculate_fee();'>
                      ...
                      <td bgcolor='#FFFFFF' class='style17'><div align='left' class='style34'>
                       <input name='other_exp' id='other_exp' required  value='";
    if (isset($c->other_exp))
        $result.=intval($c->other_exp);
    else
        $result.="";
    ...
    $result.="' size='30'  onblur='javascript:calculate_fee();'>
                    ...
                    <tr>
                      ...
                      <td bgcolor='#FFFFFF' class='style17'><div align='left' class='style34'>
                      <input name='travel_exp' id='travel_exp' required value='";
    if (isset($c->travel_exp))
        $result.=intval($c->travel_exp);
    else
        $result.="";
    ...
    $result.="' size='30'  onblur='javascript:calculate_fee();'></div></td>
                      ...
                      <td bgcolor='#FFFFFF' class='style17'><div align='left' class='style34'>
                      <input name='total_exp' id='total_exp' required value='";
    if (isset($c->total_exp))
        $result.=intval($c->total_exp);
    else
        $result.="0";
    ...
    if (isset($c->exp_prom_other))
        $result.=$c->exp_prom_other;
    $result.="' size='30'>
                      </div></td>
                    </tr>
                </table></td>
    ...
    if (isset($c->sound_phone2))
        $result.=$c->sound_phone2;
    $result.="' size='30'>
                       </div></td>
                    </tr>
                    <tr>
    ...
    if (isset($c->sound_contact))
        $result.=$c->sound_contact;
    $result.="' size='30'>
                      </div></td>
                      <td width='18%' height='36' bgcolor='#FFFFFF' class='style17'><div align='left'>
                          Email
    ...
    if (isset($c->sound_email))
        $result.=$c->sound_email;
    $result.="' size='30'></div></td>
    ...
    if (isset($c->sound_phone1))
        $result.=$c->sound_phone1;
    $result.="' size='30'>
                     </div></td>
...
function add_contract3($formdata, $make_it = 0) {
    ...
    if (!isset($formdata['id_perfomance']))
        $formdata['id_perfomance'] = 0;
...
//if (!isset($formdata['p60sten']))$formdata['p60sten']=0;else $formdata['p60sten']=1;
    ...
    if ($formdata['artist_fee'] == "")
        $formdata['artist_fee'] = 0;
    else {
        $formdata['artist_fee'] = str_replace(",", ".", $formdata['artist_fee']);
    }
    if ($formdata['admin_exp'] == "")
        $formdata['admin_exp'] = 0;
    else {
        $formdata['admin_exp'] = str_replace(",", ".", $formdata['admin_exp']);
    }
    if ($formdata['production_exp'] == "")
        $formdata['production_exp'] = 0;
    else {
        $formdata['production_exp'] = str_replace(",", ".", $formdata['production_exp']);
    }
    if ($formdata['other_exp'] == "")
        $formdata['other_exp'] = 0;
    else {
        $formdata['other_exp'] = str_replace(",", ".", $formdata['other_exp']);
    }
    if ($formdata['travel_exp'] == "")
        $formdata['travel_exp'] = 0;
    else {
        $formdata['travel_exp'] = str_replace(",", ".", $formdata['travel_exp']);
    }
    if ($formdata['total_exp'] == "")
        $formdata['total_exp'] = 0;
    else {
        $formdata['total_exp'] = str_replace(",", ".", $formdata['total_exp']);
    }
...
function viewInquiry($id) {
    ...
    if ($id > 0) {
        ...
        $query = "SELECT * from #__artists  where id=" . $inquiry->id_artist;
        $database->setQuery($query);
        $artists = $database->loadObjectList();
        foreach ($artists as $artist) {
            $result.=$artist->name;
...
<a class='style17' href='#' onclick='javascript:getpromoterInfo(" . $inquiry->id_promoter . ");'>View&nbsp;promoter</a>";
            ...
            $result.= "<a href='#' onclick='add_promoter(" . $inquiry->id . ")' title='add promoter to DB'><IMG SRC='images/user-add-24x24.png' align='absmiddle' BORDER='0' ALT='Add promoter to DB'></a>&nbsp;
<a href='#' onclick='add_promoter(" . $inquiry->id . ")' title='add promoter to DB' class='style17'>Add promoter to DB</a>";
        ...
        if ((isset($inquiry->id_promoter)) && ($inquiry->id_promoter > 0)) {
            ...
            $result.="<font color='red'><nowrap>Add promoter to DB to sign the contract!</nowrap></font>";
        }
...
//$result.="ss".$search;
    ...
    if ($search != "") {
        ...
        switch ($mode) {
            case 1: $add = " where  a.id_artist in ( select p.id from #__artists p where upper(p.name) like '%" . mb_strtoupper(mb_convert_encoding($search, 'utf8', 'UTF-8')) . "%') ";
                break;
            case 2: {
                    ...
                    $a = explode("|", $search);
                    if (strlen($a[0]) > 0)
                        $add.=" and a.id_artist in ( select a.id from #__artists a where upper(a.name) like '%" . mb_strtoupper(mb_convert_encoding($a[0], 'utf8', 'UTF-8')) . "%') ";
                    if (strlen($a[1]) > 0)
                        $add.=" and a.id_promoter in ( select p.id from #__promoters p where upper(p.name) like '%" . mb_strtoupper(mb_convert_encoding($a[1], 'utf8', 'UTF-8')) . "%') ";
                    break;
                }
            case 3: {
                    $add = " where 1>0 ";
                    $a = explode("|", $search);
                    if ($a[0] > 0)
                        $add.=" and a.contract_date >= '" . $a[0] . "' ";
                    if ($a[1] > 0)
                        $add.=" and a.contract_date <= '" . $a[1] . "' ";
                    break;
                }

            default: $add = " where  a.id_promoter in ( select p.id from #__promoters p where upper(p.name) like '%" . mb_strtoupper(mb_convert_encoding($search, 'utf8', 'UTF-8')) . "%') ";
                break;
        }
    }
...
//$result.=$query2;
    ...
    if (1 == $page)
        ...
        $links = "<div align='center'><a href='#' onclick='javascript:contract_list(" . $id . ",\"" . $search . "\"," . $mode . "," . $pp . ");'><img border=0 src='images/back-24x24.png' align='absmiddle'></a>&nbsp;<a class='style11' href='#' onclick='javascript:contract_list(" . $id . ",\"" . $search . "\"," . $mode . "," . $pp . ");'>prev&nbsp;page</a>&nbsp;";
    $links.= $paginator;
    if ($pages == $page)
        $links.="<span class='style11'>&nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span>";
    else
        $links.="&nbsp;<a class='style11' href='#' onclick='javascript:contract_list(" . $id . ",\"" . $search . "\"," . $mode . "," . $pp . ");'>next&nbsp;page</a>&nbsp;<a href='#' onclick='javascript:contract_list(" . $id . ",\"" . $search . "\"," . $mode . "," . $pp . ");'><img border=0 src='images/next-24x24.png' align='absmiddle'></a>";
    //<div align='center'>".$paginator."&nbsp; &nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span></div>
//==============================================================
...
";
    ...
    if ($ss != 0)
        $result.= "<A HREF='#' onclick='javascript:contract_list(0);' class='style11'>Active</A>";
    else
        $result.= "[<B>Active</B>]";
    $result.= "&nbsp;|&nbsp;";
    if ($ss != 1)
        $result.= "<A HREF='#' class='style11' onclick='javascript:contract_list(1);' >Deleted</A>";
    else
        $result.= "[<B>Deleted</B>]";
...
";
    ...
    if (count($contracts) > 0) {
        foreach ($contracts as $contract) {
            if ($contract->ddates > " ")
                $contract->ddates = "<br />" . $contract->ddates;
            $result.="
                  <tr>
...
";
            ...
            if ($contract->status > -1)
                $result.="<a href='#' onclick='javascript:contract_delete(" . $contract->id . ");' title='Delete this contract'><img src='images/trash-empty-24x24.png'  align='absmiddle' border='0'></a>";
            else {
...
function message_form($message_id = 0, $to_type = 0, $to_id = 0, $data = "", $who = "") {
    ...
    if ($who == 'all_promoters') {
        $to_type = 2;
        $to_id = -1;
    }
    if ($who == 'checked_promoters') {
        $to_type = 2;
        $to_id = -2;
    }
    global $database, $mosConfig_absolute_path;
    ...
    $es = email_selectors($to_type, $to_id, $data);
    if ($message_id == 0) {
        $subm = "<div id='sendbutton'><INPUT TYPE='submit' value='Send message'></div><div id='waitsend' style='display:none;'><img src='images/wait_big.gif' border=0 vspace=3 hspace=3></div>";
        $alist = array();
    ...
    if (sizeof($messages) > 0) {
        foreach ($messages as $message) {
            ...
            foreach ($attnames as $attname) {
                $n = explode("/", $attname);
                $links.="<a href='" . $attname . "' target='_blank'>" . $n[sizeof($n) - 1] . "</a>&nbsp;&nbsp;";
            }
            $ft->assign(array(
...
function change_template($id) {
    ...
    if (sizeof($messages) > 0) {
        foreach ($messages as $message) {
            $template.= $message->message_body_html;
        }
        $subject = $message->subject;
    }
...
function message_form_contract($message_id = 0, $to_type = 0, $to_id = 0, $data = 0, $id) {
    ...
    $es = email_selectors($to_type, $to_id, $data);
    if ($message_id == 0)
        $subm = "<INPUT TYPE='submit' value='Send message'>";
    else
        $subm = '';
...
//$query="select * from #__settings where id=1";
    $database->setQuery($query);
    $setting = $database->loadObjectList();
...
';
    ...
    if ($c->p60artist == 1)
        $c->p60artist = "<img src='" . $mosConfig_live_site . "/images/y.gif' />";
    else
        $c->p60artist = "<img src='" . $mosConfig_live_site . "/images/n.gif' />";
    if ($c->p60csten == 1)
        $c->p60csten = "<img src='" . $mosConfig_live_site . "/images/y.gif' />";
    else
        $c->p60csten = "<img src='" . $mosConfig_live_site . "/images/n.gif' />";
    if ($c->p60sten == 1)
        $c->p60sten = "<img src='" . $mosConfig_live_site . "/images/y.gif' />";
    else
...
function send_sms($formdata) {
    ...
    foreach ($formdata as $key => $fdata) {
        switch ($key) {
            case 'message_txt': $formdata[$key] = $fdata;
                break;
            case 'to': $formdata[$key] = $fdata;
                break;
            default:$formdata[$key] = addslashes(strip_tags($fdata));
...
function message_list($id = 0, $search = "", $page = 0) {
    ...
    if ($search != "")
        $add = " where  a.to_name like '%" . $search . "%'  OR a.to_addr like '%" . $search . "%'";
...
//$result.=$query2;
    ...
    if (1 == $page)
        ...
        $links = "<div align='center'><a href='#' onclick='javascript:messages_list(" . $id . ",\"" . $search . "\"," . $pp . ");'><img border=0 src='images/back-24x24.png' align='absmiddle'></a>&nbsp;<a class='style11' href='#' onclick='javascript:messages_list(" . $id . ",\"" . $search . "\"," . $pp . ");'>prev&nbsp;page</a>&nbsp;";
    $links.= $paginator;
    if ($pages == $page)
        $links.="<span class='style11'>&nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span>";
    else
        $links.="&nbsp;<a class='style11' href='#' onclick='javascript:messages_list(" . $id . ",\"" . $search . "\"," . $np . ");'>next&nbsp;page</a>&nbsp;<a href='#' onclick='javascript:messages_list(" . $id . ",\"" . $search . "\"," . $np . ");'><img border=0 src='images/next-24x24.png' align='absmiddle'></a>";
    //<div align='center'>".$paginator."&nbsp; &nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span></div>
    ...
    foreach ($messages as $message) {
        ...
        if ($message->to_id != 0) {
            switch ($message->to_type) {
                case "#__promoters": {
                        $link = "<a href='#msg" . $message->message_id . "' onclick='javascript:getpromoterInfo(" . $message->to_id . ");'>[promoter info]</a>";
                        break;
                    }
                default: $link = "";
            }
...
";
        ...
        if ($ss == 0)
            $result.="<a href='#' onclick='javascript:delete_message(" . $message->message_id . ");' title='Delete this message'><img src='images/trash-empty-24x24.png'  align='absmiddle' border='0'></a>";
        else
            $result.="<a href='#' onclick='javascript:message_restore(" . $message->message_id . ");' title='Restore this message'><img src='images/accept-24x24.png' align='absmiddle' border='0'></a>";
...
//$result.=$query2;
    ...
    if (1 == $page)
        ...
        $links = "<div align='center'><a href='#' onclick='javascript:sms_list(" . $id . ",\"" . $search . "\"," . $pp . ");'><img border=0 src='images/back-24x24.png' align='absmiddle'></a>&nbsp;<a class='style11' href='#' onclick='javascript:sms_list(" . $id . ",\"" . $search . "\"," . $pp . ");'>prev&nbsp;page</a>&nbsp;";
    $links.= $paginator;
    if ($pages == $page)
        $links.="<span class='style11'>&nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span>";
    else
        $links.="&nbsp;<a class='style11' href='#' onclick='javascript:sms_list(" . $id . ",\"" . $search . "\"," . $np . ");'>next&nbsp;page</a>&nbsp;<a href='#' onclick='javascript:sms_list(" . $id . ",\"" . $search . "\"," . $np . ");'><img border=0 src='images/next-24x24.png' align='absmiddle'></a>";
    //<div align='center'>".$paginator."&nbsp; &nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span></div>
...
";
    ...
    foreach ($messages as $message) {
        ...
        if ($message->to_id != 0) {
            switch ($message->to_type) {
                case "#__promoters": {
                        $link = "<a href='#msg" . $message->sms_id . "' onclick='javascript:getpromoterInfo(" . $message->to_id . ");'>[promoter info]</a>";
                        break;
                    }
                default: $link = "";
            }
...
";
        ...
        if ($ss == 0)
            $result.="<a href='#' onclick='javascript:delete_sms(" . $message->sms_id . ");' title='Delete this message'><img src='images/trash-empty-24x24.png'  align='absmiddle' border='0'></a>";
        else
            $result.="<a href='#' onclick='javascript:sms_restore(" . $message->sms_id . ");' title='Restore this message'><img src='images/accept-24x24.png' align='absmiddle' border='0'></a>";
...
function email_selectors($type = 0, $whos_id = 0, $data = "") {
    ...
    if (0 != $type) {
        ...
        $result.="<INPUT TYPE='hidden' name='to_type' id='to_type' value=''>";
    $nn = "";
    if (0 != $whos_id) {
        $query = "
...
select * from " . $typs[$type] . " where id=" . $whos_id;
        ...
        if (strlen($email->phone2) > 1)
            $ee .= preg_replace("/[ ()-]/", "", $email->phone2);
        else
            $ee .= preg_replace("/[ ()-]/", "", $email->phone1);
        $nn = $email->name;
    }
...
select * from " . $typs[$type] . " where status > 0";
        ...
        $ems = $database->loadObjectList();
        foreach ($ems as $em) {
            if ((strlen($em->phone2) > 1))
                $ee .= preg_replace("/[ ()-]/", "", $em->phone2) . ", ";
            else
                $ee .= preg_replace("/[ ()-]/", "", $em->phone1) . ", ";
        }
        $ee = substr($ee, 0, strlen($ee) - 2);
    }
    if ($whos_id == -2) {
        ...
        $ee = "";
        foreach ($promoters as $promoter) {
            if ((strlen($promoter->phone2) > 1))
                $ee.=" " . $promoter->name . " <" . preg_replace("/[ ()-]/", "", $promoter->phone2) . ">, ";
            else
                $ee.=" " . $promoter->name . " <" . preg_replace("/[ ()-]/", "", $promoter->phone1) . ">, ";
        }
        $ee = (substr($ee, 0, strlen($ee) - 2));
    }
    if ((strlen($data) > 11))
...
<table width='95%' border='0' align='center' cellpadding='0' cellspacing='1'>
 ...
 <input type='text' name='search_term' id='search_term' size='40' maxlenght='40' required='1'>
    ...
    for ($i = 0; $i < count($nam); $i++) {
        if ($mode != $i)
            $result.="<option value='" . $i . "'>" . $nam[$i] . "</option>";
        else
            $result.="<option value='" . $i . "' selected>" . $nam[$i] . "</option>";
    }
...
//case 4: {$result.=viewSchedules($fdatas['id_artist'],date("Y"),"","",$fdatas['week_number']);break;}
        case 4: {
                $result.=get_busyday($fdatas['id_artist'], date("Y"));
                break;
            }
        case 5: {
        ...
        case 7: {
                $result.=contracts_list(0, $fdatas['promoter_name'], 0);
                break;
            }
        case 8: {
                $result.=contracts_list(0, $fdatas['artist_name'], 1);
                break;
            }
        case 9: {
                $result.=contracts_list(0, $fdatas['artist_name'] . "|" . $fdatas['promoter_name'], 2);
                break;
            }
        case 10: {
                $result.=contracts_list(0, $fdatas['date_from'] . "|" . $fdatas['date_to'], 3);
                break;
            }
        case 11: {
        ...
        case 12: {
                $result.=promoters_list(0, $fdatas['search_term'], 0, $fdatas['weeknum'], $fdatas['country'], $fdatas['town']);
                break;
            }
        case 0: {
                $result.=agencys_list(0, $fdatas['search_term']);
...
//case 4: {$result.=viewSchedules($fdatas['id_artist'],date("Y"),"","",$fdatas['week_number']);break;}
        ...
        case 6: {
                $result.=message_list(0, $what);
                break;
            }
        case 12: {
                $result.=promoters_list(0, "", 0, $what, $country, $town);
                break;
            }
...
function check_name($susp, $inn, $id = 0, $link, $silent = 0) {
    ...
    if (count($maybe_names) > 1) {
        ...
        foreach ($maybe_names as $maybe_name) {
            ...
            if (strlen($maybe_name) > 1)
                $add.=" OR a.name like '%" . $maybe_name . "%' ";
        }
        $add.=") ";
    } else {
        $maybe_names = $susp;
    ...
    if ($counts > 0) {
        if ($silent)
            $objResponse->addScript("if (!(confirm('We have some dupplicates! Do you want to continue?'))) hideInfo();");
...
function agency_list($agent_id, $search = "", $page = 0) {
    ...
    if ($page == 'undefined')
        $page = 0;
    if ($search == 'undefined')
        $search = "";setcookie('prev', stripslashes($_COOKIE['now']));
...
//$result.=$query;
    ...
    if (1 == $page)
        ...
        $links = "<div align='center'><a href='#' onclick='javascript:agency_list(" . $id . ",\"" . $search . "\"," . $pp . ");'><img border=0 src='images/back-24x24.png' align='absmiddle'></a>&nbsp;<a class='style11' href='#' onclick='javascript:agency_list(" . $id . ",\"" . $search . "\"," . $pp . ");'>prev&nbsp;page</a>&nbsp;";
    $links.= $paginator;
    if ($pages == $page)
        $links.="<span class='style11'>&nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span>";
    else
        $links.="&nbsp;<a class='style11' href='#' onclick='javascript:agency_list(" . $id . ",\"" . $search . "\"," . $np . ");'>next&nbsp;page</a>&nbsp;<a href='#' onclick='javascript:agency_list(" . $id . ",\"" . $search . "\"," . $np . ");'><img border=0 src='images/next-24x24.png' align='absmiddle'></a>";
    //<div align='center'>".$paginator."&nbsp; &nbsp;next&nbsp;page&nbsp;<img border=0 src='images/next-24x24.png' align='absmiddle'></span></div>
...
";
            ...
            if ($search)
                ...
                if ($ss != 0)
                    $result.= "<A HREF='#' onclick='javascript:agency_list(0);' class='style11'>Active</A>";
                else
                    $result.= "[<B>Active</B>]";
                $result.= "&nbsp;|&nbsp;";
                if ($ss != 1)
                    $result.= "<A HREF='#' class='style11' onclick='javascript:agency_list(1);' >Deleted</A>";
                else
                    $result.= "[<B>Deleted</B>]";
            }
            $result.="</td></tr>";
...
<tr><td width='45%' height='35' bgcolor='white'>&nbsp;";
        if ($search)
            $result.="<input type='checkbox' align='absmiddle' id='agency_" . $agent->id . "' prop='mail' boxtype='agency' email='" . $agent->email . "'>&nbsp;";
        $result.="
<a href='#' onclick='javascript:editagencyInfo(" . $agent->id . ");' title='Edit info on this Agency'>
<img src='images/blog-post-edit-24x24.png'  align='absmiddle' border='0'></a>&nbsp;";

        if ($ss == 0)
            $result.="<a href='#' onclick='javascript:agency_delete(" . $agent->id . ");' title='Delete this Agency'><img src='images/trash-empty-24x24.png'  align='absmiddle' border='0'></a>";
        else {
            $result.="<a href='#' onclick='javascript:agency_restore(" . $agent->id . ");' title='Restore this Agency'><img src='images/accept-24x24.png' align='absmiddle' border='0'></a>";
...
function agency_delete($id, $mode = 0) {
    ...
    if ($mode == 1)
        $query = "delete from `#__agency` where `id`=" . $id;
//echo $query;
    $database->setQuery($query);
    $database->query();
    $objResponse = new xajaxResponse('UTF-8');
    ...
    function agent_delete(id, mode) {
        try {
            if (confirm("Are you sure to delete this agent??")) {
                ...
                xajaxDebug = false, xajaxStatusMessages = false, xajaxWaitCursor = true, xajaxDefinedGet = 0, xajaxDefinedPost = 1;
                if (mode == 1) {
                    if (confirm("Are you sure to finally delete agent with ID=" + id + " ??"))
                        main_agent_delete(id, mode);
                } else
                    main_agent_delete(id, 0);
            }
        } catch (e) {
            alert(e);
        }
    ...
    function agency_delete(id, mode) {
        try {
            if (confirm("Are you sure to delete this agency??")) {
                ...
                xajaxDebug = false, xajaxStatusMessages = false, xajaxWaitCursor = true, xajaxDefinedGet = 0, xajaxDefinedPost = 1;
                if (mode == 1) {
                    if (confirm("Are you sure to finally delete Agency with ID=" + id + " ??"))
                        main_agency_delete(id, mode);
                } else
                    main_agency_delete(id, 0);
            }
        } catch (e) {
            alert(e);
        }
    ...
    function inquiry_delete(id, mode) {
        try {
            if (confirm("Are you sure to delete this inquiry??")) {
                ...
                xajaxDebug = false, xajaxStatusMessages = false, xajaxWaitCursor = true, xajaxDefinedGet = 0, xajaxDefinedPost = 1;
                if (mode == 1) {
                    if (confirm("Are you sure to finally delete inquiry with ID=" + id + " ??"))
                        main_inquiry_delete(id, mode);
                } else
                    main_inquiry_delete(id, 0);
            }
        } catch (e) {
            alert(e);
        }
    ...
    function promoter_delete(promoter_id, mode) {
        try {
            if (confirm("Are you sure to delete this promoter??")) {
                ...
                xajaxDebug = false, xajaxStatusMessages = false, xajaxWaitCursor = true, xajaxDefinedGet = 0, xajaxDefinedPost = 1;
                if (mode == 1) {
                    if (confirm("Are you sure to finally delete promoter with ID=" + promoter_id + " ??"))
                        main_promoter_delete(promoter_id, mode);
                } else
                    main_promoter_delete(promoter_id, 0);
            }
        } catch (e) {
            alert(e);
        }
    ...
    function artist_delete(artist_id, mode) {
        try {
            ...
            if (confirm("Are you sure to delete this artist??")) {
                ...
                xajaxDebug = false, xajaxStatusMessages = false, xajaxWaitCursor = true, xajaxDefinedGet = 0, xajaxDefinedPost = 1;
                if (mode == 1) {
                    if (confirm("Are you sure to finally delete artist with ID=" + artist_id + " ??"))
                        main_artist_delete(artist_id, mode);
                } else
                    main_artist_delete(artist_id, 0);
            }
        } catch (e) {
            alert(e);
        }
        ...
        try {
            ...
            for (i = 0; i < boxes.length; i++) {
                if (boxes[i].checked == true)
                {
                    ...
                    {
                        if (!email)
                            email = boxes[i].getAttribute("email");
                        else
                            email = email + ", " + boxes[i].getAttribute("email");
                    }
...
//alert(boxes[i].getAttribute("phone"));
                {
                    if ((boxes[i].getAttribute("phone") != "undefined") && (boxes[i].getAttribute("phone") != null))
                    {
                        if (!sms) {
                            ...
                            sms = sms + ", " + boxes[i].getAttribute("phone");
                    }
...
// alert (boxtype + ' | ' + ids + ' | ' + values);
        try {
            main_check_box_saver(boxtype, ids, values)
        }
        catch (e) {
            alert(e);
        }
...
// document.getElementById('attachedfiles').style.visibility = 'visible';
    ...
    function check_non_empty() {
        ...
        if (((m * 1) * (n * 1)) > 0) {
            ...
            alert('please, select artist & promoter from list above');
            return false;
    ...
    function calculate_fee()
    {
        ...
        if (document.getElementById('artist_fee').value == '')
            var a = 0;
        else
            var a = FTS((document.getElementById('artist_fee').value), 0) * 1;
//document.getElementById('artist_fee').value= FTS(a,0)*1;
        if (document.getElementById('admin_exp').value == '')
            var b = 0;
        else
            var b = FTS((document.getElementById('admin_exp').value), 0) * 1;
//document.getElementById('admin_exp').value= FTS(b,0)*1
        if (document.getElementById('production_exp').value == '')
            var c = 0;
        else
            var c = FTS((document.getElementById('production_exp').value), 0) * 1;
//document.getElementById('production_exp').value= FTS(c,0)*1
        if (document.getElementById('other_exp').value == '')
            var d = 0;
        else
            var d = FTS((document.getElementById('other_exp').value), 0) * 1;
//document.getElementById('other_exp').value= FTS(d,0)*1
        if (document.getElementById('travel_exp').value == '')
            var e = 0;
        else
            var e = FTS((document.getElementById('travel_exp').value), 0) * 1;
//document.getElementById('travel_exp').value= FTS(e,0)*1
    ...
    function processScheduleFieldsState() {

        var scheduleFields = ['form_pressconf',
            'form_dinner',
            'form_getintime',
            'form_soundcheck',
            'form_doorsopen',
        ...
        if (document.getElementById('freeday').checked) {
            scheduleFields.forEach(function(entry) {
                if (document.getElementById(entry).tagName === 'TABLE') {
                    setTableActive(entry, false);
                } else {
                    setElementActive(document.getElementById(entry), false);
                }
            });
            ...
            scheduleFields.forEach(function(entry) {
                ...
                if (document.getElementById(entry).tagName === 'TABLE') {
                    setTableActive(entry, true);
                } else {
                    setElementActive(document.getElementById(entry), true);
                }
            });
