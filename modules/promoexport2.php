...
$hhh=array("ID","Promoter","Contact person","Weekend number","Address 1","Address 2","City code (ZIP)","Town","Location","Country","Local phone","Cell phone","e-mail", "Website","Сapacity","Type_of_music","Comments");
$i=0;
foreach ($hhh as $hh){
 $objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow($i,1,$hh);$i++;
}
$query="select p.id, p.name, p.contact_person, p.weeknum , p.street_addr1, p.street_addr2, p.city_code, p.town, p.location, ( select c.name from demo3_countries c where c.id= p.country) as country, p.phone1, p.phone2, p.email, p.website, p.capacity, p.typemusic , p.comments FROM nor_promoters p WHERE 1=1";
