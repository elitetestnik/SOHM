	...
	*/
	function writePagesLinks( $link ) {
		...
		if ($this_page > 1) {
			...
			$txt .= '<a href="'. sefRelToAbs( "$link&amp;limitstart=0" ) .'" class="pagenav" title="первая страница">&lt;&lt; '. _PN_START .'</a> ';
			$txt .= '<a href="'. sefRelToAbs( "$link&amp;limitstart=$page" ) .'" class="pagenav" title="предыдущая страница">&lt; '. _PN_PREVIOUS .'</a> ';
		} else {
		...
		if ($this_page < $total_pages) {
			...
			$txt .= '<a href="'. sefRelToAbs( $link .'&amp;limitstart='. $page ) .' " class="pagenav" title="следующая страница">'. _PN_NEXT .' &gt;</a> ';
			$txt .= '<a href="'. sefRelToAbs( $link .'&amp;limitstart='. $end_page ) .' " class="pagenav" title="последняя страница">'. _PN_END .' &gt;&gt;</a>';
		} else {
