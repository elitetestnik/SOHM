...
if (in_array( 'globals', array_keys( array_change_key_case( $_REQUEST, CASE_LOWER ) ) ) ) {
	die( 'Фатальная ошибка. Попытка хака через глобальные переменные.' );
}
if (in_array( '_post', array_keys( array_change_key_case( $_REQUEST, CASE_LOWER ) ) ) ) {
	die( 'Фатальная ошибка.  Попытка хака через POST.' );
}
...
class mosHTML {
  ...
  function writableCell( $folder ) {
  	...
  	echo '<td align="left">';
  	echo is_writable( "../$folder" ) ? '<b><font color="green">Доступен на запись</font></b>' : '<b><font color="red">Не доступен на запись</font></b>' . '</td>';
  	echo '</tr>';
  }
...
class mosCategory extends mosDBTable {
	...
	*/
	...
	function check() {
		...
		if (trim( $this->title ) == '') {
			$this->_error = "Ваша категория должна иметь заголовок.";
			return false;
		}
		if (trim( $this->name ) == '') {
			$this->_error = "Ваша категория должна иметь имя.";
			return false;
		}
		...
		if ($xid && $xid != intval( $this->id )) {
			$this->_error = "Категория с таким именем уже существует, дайте другое имя.";
			return false;
		}
		return true;
	}
...
class mosSection extends mosDBTable {
	...
	*/
	...
	function check() {
		// check for valid name
		if (trim( $this->title ) == '') {
			$this->_error = "Ваш раздел должен иметь заголовок.";
			return false;
		}
		if (trim( $this->name ) == '') {
			$this->_error = "Ваш раздел должен иметь имя.";
			return false;
		}
		// check for existing name
		...
		$xid = intval( $this->_db->loadResult() );
		if ($xid && $xid != intval( $this->id )) {
			$this->_error = "Раздел с таким именем уже существует, дайте другое имя.";
			return false;
		}
		return true;
	}
...
class mosModule extends mosDBTable {
	...
	*/
	...
	function check() {
		// check for valid name
		if (trim( $this->title ) == '') {
			$this->_error = "Ваш модульдолжен иметь заголовок";
			return false;
		}
...
class mosSession extends mosDBTable {
	...
	*/
	...
	function update( $updateNulls=false ) {
		...
		if( !$ret ) {
			$this->_error = strtolower(get_class( $this ))."::запись неудачна <br />" . $this->_db->stderr();
			return false;
		} else {
			return true;
		}
...
*/
function mosWarning($warning, $title='Предупреждение Мамбо') {
	global $mosConfig_live_site;
...
class mosAdminMenus {
	...
	*/
	function Target( &$row ) {
		$click[] = mosHTML::makeOption( '0', 'То же окно с кнопаками навигации' );
		$click[] = mosHTML::makeOption( '1', 'Новое окно с кнопаками навигации' );
		$click[] = mosHTML::makeOption( '2', 'То же окно без кнопок навигации' );
	...
	*/
	function Images( $name, &$active, $javascript=NULL, $directory=NULL ) {
		...
		$images = array(  mosHTML::makeOption( '', '- Выбор изображения -' ) );
	...
	*/
	function UserSelect( $name, $active, $nouser=0, $javascript=NULL, $order='name' ) {
		...
		$database->setQuery( $query );
		if ( $nouser ) {
			$users[] = mosHTML::makeOption( '0', '- Нет юзера -' );
	...
	*/
	function ComponentCategory( $name, $section, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
		...
		if ( count( $categories ) < 1 ) {
			mosRedirect( 'index2.php?option=com_categories&section='. $section, 'Вы должны сначала создать категорию.' );
		}
...
class mosCommonHTML {
	...
	function ContentLegend( ) {
		...
		<tr align="center">
			<td>
			<img src="images/publish_y.png" width="12" height="12" border="0" alt="Просрочен" />
			</td>
			<td>
			Опубликовано, но <u>Ожидает</u> |
			</td>
			<td>
			<img src="images/publish_g.png" width="12" height="12" border="0" alt="Виден" />
			</td>
			<td>
			Опубликовано и <u>Активно</u> |
			</td>
			<td>
			<img src="images/publish_r.png" width="12" height="12" border="0" alt="Закончен" />
			</td>
			<td>
			Опубликовано, но <u>Просрочено</u> |
			</td>
			<td>
			<img src="images/publish_x.png" width="12" height="12" border="0" alt="Закончен" />
			</td>
			<td>
			Не опубликовано
			</td>
		...
		<tr>
			...
			Кликните по иконке для переключения состояния.
			</td>
		</tr>
		</table>
		<?php
	...
	function menuLinksContent( &$menus ) {
		...
		foreach( $menus as $menu ) {
			...
			<tr>
				...
				<td>
				<a href="javascript:go2( 'go2menu', '<?php echo $menu->menutype; ?>' );" title="Идти в меню">
			...
			<tr>
				...
				<td>
				<strong>
				<a href="javascript:go2( 'go2menuitem', '<?php echo $menu->menutype; ?>', '<?php echo $menu->id; ?>' );" title="Идти в пункт меню">
				<?php echo $menu->name; ?>
				</a>
				</strong>
				</td>
			...
			<tr>
				...
				switch ( $menu->published ) {
					case -2:
						echo '<font color="red">Удалено</font>';
						break;
					case 0:
						echo 'UnPublished';
						break;
	...
	*/
	...
	function PublishedProcessing( &$row, $i ) {
		...
		$action	= $row->published ? 'Снять с публикации' : 'Опубликовать';
