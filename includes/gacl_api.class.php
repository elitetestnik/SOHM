...
class gacl_api extends gacl {
	 ...
	 */
	var $_items_per_page = 1000;
	var $_max_select_box_items = 1000;
	var $_max_search_return_items = 1000;
	...
	function return_page($url="") {
		...
		if (empty($url) AND !empty($_SERVER[HTTP_REFERER])) {
			$this->debug_text("return_page(): URL не установлен, используется reffer!");
	...
	function get_group_id($name = null, $group_type = 'ARO') {
		...
		if (empty($name) ) {
			$this->debug_text("get_group_id(): имя ($name) пустое, это требуется");
			return false;
		}
		...
		if ($row_count > 1) {
			$this->debug_text("get_group_id(): Возвращено $row_count строк, должна быть только одна. Пожалуйста сделайте имя уникальным.");
			return false;
		}

		if ($row_count == 0) {
			$this->debug_text("get_group_id(): Возвращено $row_count строк");
			return false;
		}
	...
	function get_group_name($group_id = null, $group_type = 'ARO') {
		...
		if (!$group_id) {
			$this->debug_text("get_group_name(): group_id ($group_id) пустой, он обязателен");
			return false;
		}
		...
		if ($row_count > 1) {
			$this->debug_text("get_group_name(): Возвращено $row_count строк, должна быть только одна. Пожалуйста сделайте имя уникальным.");
			return false;
		}

		if ($row_count == 0) {
			$this->debug_text("get_group_name(): Возвращено $row_count строк");
			return false;
		}
	...
	function get_group_children($group_id, $group_type = 'ARO', $recurse = 'NO_RECURSE') {
		...
		if (empty($group_id)) {
			$this->debug_text("get_group_children(): ID ($group_id) пусто, но обязательно");
			return FALSE;
		}
	...
	function get_group_parents($group_id, $group_type = 'ARO', $recurse = 'NO_RECURSE') {
		...
		if (empty($group_id)) {
			$this->debug_text("get_group_parents(): ID ($group_id) пусто, но обязательно");
			return FALSE;
		}
	...
	function add_group($name, $parent_id=0, $group_type='ARO') {
		...
		if (empty($name)) {
			$this->debug_text("add_group(): name ($name) OR parent id ($parent_id) пусто, хотя обязательно");
			return false;
		}
		...
		if ($parent_id == 0) {
			...
			if (count( $rs ) > 0) {
				$this->debug_text('add_group (): Корневая группа уже существует.');
			...
			if (empty($parent_id)) {
				$this->debug_text("add_group (): parent id ($parent_id) пуста, хотя обязательна");
				return FALSE;
			}
			...
			if (empty($rows)) {
				$this->debug_text('add_group (): Parent ID: '. $parent_id .' не найден.');
				// <mos> $this->db->RollBackTrans();
				return FALSE;
			}
			$row = $rows[0];
		...
		if (!$rs) {
			$this->debug_db('add_group: вставка записи');
			// <mos> $this->db->RollBackTrans();
			return FALSE;
		}
