...
<?php
/*******
* AndyR - патч "кракозяблей", данных подкачанных из XML-файлов,
* при отображении параметров меню в кодировке UTF-8.
* Применяется в версии Мамбо 4.5.2.х
*
* 29.05.2005 - A&AP Programmer -
*
**/
	...
	function render( $name='params' ) {
	    ...
	    if (is_object( $this->_xmlElem )) {
			...
			foreach ($element->childNodes as $param) {
				...
				If ( detect_utf8($result[0])) {
          $result[0] = utf8_win1251 ($result[0]);
				}
				If ( detect_utf8($result[1])) {
          $result[1] = utf8_win1251 ($result[1]);
				}
				If ( detect_utf8($result[2])) {
          $result[2] = utf8_win1251 ($result[2]);
				}
...
*/
	...
	*/
	function _form_mos_section( $name, $value, &$node, $control_name ) {
		...
		array_unshift( $options, mosHTML::makeOption( '0', '- Выбор раздела содержимого -' ) );
	...
	*/
	function _form_mos_category( $name, $value, &$node, $control_name ) {
		...
		$database->setQuery( $query );
		$options = $database->loadObjectList();
		array_unshift( $options, mosHTML::makeOption( '0', '- Выбор категории содержимого -' ) );
	...
	*/
	function _form_imagelist( $name, $value, &$node, $control_name ) {
		...
		if ( !$node->getAttribute( 'hide_default' ) ) {
			array_unshift( $options, mosHTML::makeOption( '', '- Картинка по умолчанию -' ) );
		}
