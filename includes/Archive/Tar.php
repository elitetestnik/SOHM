...
class Archive_Tar extends PEAR
{
        ...
        if ($this->_compress) { // assert zlib or bz2 extension support
            ...
            if (!extension_loaded($extname)) {
                die("Расширение '$extname' не найдено.\n".
                    "Убедитесь, что ваша версия PHP скомпилирована ".
                    "с поддержкой '$extname'\n");
                return false;
            }
        }
    }
    ...
    */
        ...
        while ($i<$v_size) {
            ...
            switch ($v_att_list[$i]) {
                ...
                case ARCHIVE_TAR_ATT_SEPARATOR :
                    ...
                    if (($i+1) >= $v_size) {
                        $this->_error('Недопустимое количество параметров для атрибута ARCHIVE_TAR_ATT_SEPARATOR');
                        return false;
                    }
                ...
                default :
                    $this->_error('Неизвестный код атрибута '.$v_att_list[$i].'');
                    return false;
            }
    ...
    function _openWrite()
    {
        if ($this->_compress_type == 'gz')
            ...
            $this->_error('Неизвестный или отсутствующий тип сжатия ('.$this->_compress_type.')');

        if ($this->_file == 0) {
            $this->_error('Не могу открыть в режиме записи \''.$this->_tarname.'\'');
            return false;
        }
    ...
    function _openRead()
    {
        if (strtolower(substr($this->_tarname, 0, 7)) == 'http://') {
          ...
          if ($this->_temp_tarname == '') {
              ...
              if (!$v_file_from = @fopen($this->_tarname, 'rb')) {
                $this->_error('Не могу открыть в режиме чтения \''.$this->_tarname.'\'');
                $this->_temp_tarname = '';
                return false;
              }
              if (!$v_file_to = @fopen($this->_temp_tarname, 'wb')) {
                $this->_error('Не могу открыть в режиме записи \''.$this->_temp_tarname.'\'');
                $this->_temp_tarname = '';
                return false;
              }
        ...
        if ($this->_file == 0) {
            $this->_error('Не могу открыть в режиме чтения \''.$v_filename.'\'');
            return false;
        }
    ...
    function _openReadWrite()
    {
        ...
        if ($this->_file == 0) {
            $this->_error('Не могу открыть в режиме чтения/записи \''.$this->_tarname.'\'');
            return false;
        }
      ...
      for ($j=0; ($j<count($p_list)) && ($v_result); $j++) {
        ...
        if (@is_dir($v_filename)) {
            if (!($p_hdir = opendir($v_filename))) {
                $this->_warning("Каталог '$v_filename' не может быть прочитан");
                continue;
            }
    ...
    function _addFile($p_filename, &$p_header, $p_add_dir, $p_remove_dir)
    {
      ...
      if (is_file($p_filename)) {
          if (($v_file = @fopen($p_filename, "rb")) == 0) {
              $this->_warning("Не могу открыть файл '$p_filename' в режиме бинарного чтения");
              return true;
          }
    ...
    function _readHeader($v_binary_data, &$v_header)
    {
        ...
        if ($v_header['checksum'] != $v_checksum) {
            ...
            $this->_error('Неверная контрольная сумма "'.$v_data['filename'].'" : '.$v_checksum.' вычислено, '.$v_header['checksum'].' должно быть');
            return false;
        }
    ...
    */
        ...
        While (strlen($v_binary_data = $this->_readBlock()) != 0)
        {
          ...
          if ($v_header['filename'] == $p_filename) {
              if ($v_header['typeflag'] == "5") {
                  $this->_error('Невозможно извлечь в строку элемент каталога {'.$v_header['filename'].'}');
                  return NULL;
              } else {
                  $n = floor($v_header['size']/512);
                  for ($i=0; $i<$n; $i++) {
    ...
    switch ($p_mode) {
      ...
      break;
      default :
        $this->_error('Неверный режим извлечения ('.$p_mode.')');
        return false;
    }
      ...
      if (($v_extract_file) && (!$v_listing))
      {
        ...
        if (file_exists($v_header['filename'])) {
          if ((@is_dir($v_header['filename'])) && ($v_header['typeflag'] == '')) {
            $this->_error('Файл '.$v_header['filename'].' уже существует как каталог');
            return false;
          }
        ...
        if ($v_extract_file) {
          if ($v_header['typeflag'] == "5") {
            if (!@file_exists($v_header['filename'])) {
                if (!@mkdir($v_header['filename'], 0777)) {
                    $this->_error('Не могу создать каталог {'.$v_header['filename'].'}');
                    return false;
                }
            }
          } else {
              if (($v_dest_file = @fopen($v_header['filename'], "wb")) == 0) {
                  $this->_error('Ошибка при открытии {'.$v_header['filename'].'} на запись в бинарном режиме');
                  return false;
              } else {
                  $n = floor($v_header['size']/512);
                  for ($i=0; $i<$n; $i++) {
          ...
          if (filesize($v_header['filename']) != $v_header['size']) {
              $this->_error('Разархивированный файл '.$v_header['filename'].' имеет неверный размер \''.filesize($v_filename).'\' ('.$v_header['size'].' ожидалось). Архив может быть нарушен.');
              return false;
          }
          }
        } else {
...
//#############'#
            ...
            if ($v_temp_tar == 0) {
                $this->_error('Не могу открыть файл \''.$this->_tarname.'.tmp\' в бинарном режиме');
...
//'#
    ...
    function _dirCheck($p_dir)
    {
        ...
        if (!@mkdir($p_dir, 0777)) {
            $this->_error("Невозможно создать каталог '$p_dir'");
            return false;
        }
